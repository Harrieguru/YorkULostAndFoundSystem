...
router.get("/login-auth", async (req, res) => {
  ...
  try {
    ...
    if (data.rows.length === 0) {
      return res
        .status(401)
        .json({ incorrectUsername: "User not in database" });
    }
    ...
    if (isPassValid === false) {
      return res
        .status(401)
        .json({ incorrectPassword: "password not in database" });
    }

    res.json({
      validPassword: isPassValid,
      enteredPassword: pass,
      status: "password has matched its hash",
    });
  } catch (err) {
    console.error(err);
